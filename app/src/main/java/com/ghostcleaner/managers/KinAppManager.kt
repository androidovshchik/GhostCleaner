package com.ghostcleaner.managers
...
import com.ghostcleaner.models.*
import kotlinx.coroutines.*
import org.json.JSONObject

class KinAppManager(private val context: Context) {
    ...
    fun isBillingSupported(productType: KinAppProductType): Boolean =
        billingService?.isBillingSupported(
            KINAPP_API_VERSION,
            context.packageName,
            productType.value
        ) == KINAPP_RESPONSE_RESULT_OK

    suspend fun fetchProductsAsync(
        productIds: ArrayList<String>,
        productType: KinAppProductType
    ): Deferred<List<KinAppProduct>?> {
        return GlobalScope.async(Dispatchers.Main, CoroutineStart.DEFAULT) {
            val bundle = Bundle()
            bundle.putStringArrayList(GET_ITEM_LIST, productIds)
            try {
                val responseBundle = billingService?.getSkuDetails(
                    KINAPP_API_VERSION,
                    this@KinAppManager.context.packageName,
                    productType.value,
                    bundle
                )
    ...
    fun purchase(activity: Activity, productId: String, productType: KinAppProductType) {
        try {
            val responseBundle = billingService?.getBuyIntent(
                KINAPP_API_VERSION,
                context.packageName,
                productId,
                productType.value,
            ...
            if (result == KINAPP_RESPONSE_RESULT_OK) {
                val pendingIntent =
                    responseBundle?.getParcelable<PendingIntent>(RESPONSE_BUY_INTENT)
            ...
            if (resultCode == Activity.RESULT_OK) {
                ...
                if (purchaseData != null) {
                    ...
                    if (purchase.productId.startsWith(TEST_PURCHASE_PREFIX) ||
                        (dataSignature != null && SecurityManager.verifyPurchase(
                            developerPayload,
                            purchaseData,
                            dataSignature
                        ))
    ...
    suspend fun consumePurchaseAsync(purchase: KinAppPurchase): Deferred<Boolean> {
        return GlobalScope.async(Dispatchers.Main, CoroutineStart.DEFAULT) {
            try {
                val response = billingService?.consumePurchase(
                    KINAPP_API_VERSION,
                    this@KinAppManager.context.packageName,
    ...
    private fun getProduct(productData: String): KinAppProduct {
        ...
        return KinAppProduct(
            ...
            type = if (inappProduct.optString("type")
                    .equals(SUBS_TYPE, ignoreCase = true)
            ) KinAppProductType.SUBSCRIPTION else KinAppProductType.INAPP
        )
    }
    ...
    private fun getPurchase(purchaseData: String): KinAppPurchase {
        ...
        return KinAppPurchase(
            ...
            purchaseState = KinAppPurchaseState.values()[item.optInt(
                "purchaseState",
                KinAppPurchaseState.CANCELED.value
            )],
    ...
    private fun retrievePurchases(
        purchases: MutableList<KinAppPurchase>,
        productType: KinAppProductType,
        continuationToken: String?
    ): MutableList<KinAppPurchase> {
        val responseBundle = billingService?.getPurchases(
            KINAPP_API_VERSION,
            context.packageName,
            productType.value,
            continuationToken
        ...
        if (getResult(responseBundle, RESPONSE_CODE) == KINAPP_RESPONSE_RESULT_OK) {
            val inappPurchases = responseBundle?.getStringArrayList(
            ...
            if (inappPurchases != null) {
                ...
                val newContinuationToken = responseBundle.getString(
